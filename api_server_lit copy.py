...
# ============== Inference parameter extraction (same rules as api_server.py) ==============
...
def _extract_params(data: dict) -> dict:
    ...
    # Prevent beam search + sampling combo (unsafe under accelerated inference)
    if params.get("do_sample", True) and params.get("num_beams", 3) > 1:
        params["num_beams"] = 1

    # Drop emotion reference paths that don't exist on this server
...
class IndexTTS2LitAPI(ls.LitAPI):
    ...
    One model instance per worker; LitServe serializes requests within a
    worker, which is exactly the single-GPU guarantee the semaphore queue in
    api_server.py provides — minus the asyncio/Gradio overhead.

    NOTE: LitServe runs workers in SEPARATE processes (this object is pickled
    into them, and `if __name__ == "__main__"` does not re-run there). All
    config must therefore live on `self`, never in module globals like ARGS.
    """

    def __init__(self, config: argparse.Namespace, **kwargs):
        super().__init__(**kwargs)
        # plain-dict copy: argparse.Namespace pickles fine, dict is safer still
    ...
    def setup(self, device):
        ...
        dev = None if cfg["device"] == "auto" else cfg["device"]
        logger.info(f"[worker] Loading IndexTTS2 from {model_dir} on {device} (fp16={cfg['fp16']})...")
        ...
        logger.info(f"[worker] IndexTTS2 ready in {time.perf_counter() - t0:.1f}s")
...
def _register_compat_routes(server: "ls.LitServer"):
    ...
    async def health():
        return {
            "status": "healthy",
            "model_loaded": True,  # workers load in setup(); LitServe won't serve before that
            "backend": "litserve",
            "timestamp": time.time(),
            "concurrency": {
    ...
    async def upload_speaker(
        ...
        try:
            ...
            meta[voice_id] = {
                ...
                "created_at": time.time(),
                # embedding caches inside IndexTTS2 on first synthesis with this voice
                "embedding_cached": False,
            }
            _save_meta(meta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workers", type=int, default=int(os.environ.get("INDEXTTS_WORKERS", "1")),
                        help="LitServe workers per device. >1 loads N model copies (needs VRAM x N) for parallel inference.")
